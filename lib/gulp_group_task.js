...
class GulpGroupTask extends GulpDestMixin {
  ...
  constructor(name, group){
    ...
    this.parent = group
    this.globs = []
    this.Gulp = require('./gulp').Gulp
    this.gulp = this.Gulp.gulp
  }
  ...
  createCopyTask(){
    ...
    this.gulp.task(`${this.parent.name}:${this.name}`, copy_fn)
  ...
  createWebpackTask( options = {}, webpack_config ){
    ...
    this.gulp.task(`${this.parent.name}:${this.name}`, webpack_fn)
  ...
  createBabelTask( babel_options = {presets: ['es2015']} ){
    ...
    this.gulp.task(`${this.parent.name}:${this.name}`, babel_fn)
  ...
  createBabelSourceMapTask( babel_options = {presets: ['es2015']} ){
    ...
    this.gulp.task(`${this.parent.name}:${this.name}`, babel_smap_fn)
    return true
  }
  ...
  createSassTask(){
    ...
    this.gulp.task(`${this.parent.name}:${this.name}`, sass_fn)
  ...
  createSassSourceMapTask(){
    ...
    let tsk = this.gulp.task(`${this.parent.name}:${this.name}`, sass_smap_fn)
  ...
  createShellTask(commands){
    this.gulp.task(`${this.parent.name}:${this.name}`, shell.task(commands))
    return true
  }

  // Run a gulp-sequence of tasks
  createSequence(...args){
    this.gulp.task(`${this.parent.name}:${this.name}`, gulpSequence(...args))
    return true
  }
  ...
  createCustom(fn){
    this.gulp.task(`${this.parent.name}:${this.name}`, fn)
    return true
  }
