...
class GulpGroup extends GulpDestMixin {
  ...
  constructor(name, groups){
    ...
    this.parent = groups
    this.tasks = {}
    this._groups = new (require('./gulp_groups').GulpGroups)('', this)
    this.Gulp = require('./gulp')
  }

  // Our main job is to hold tasks
  task(name){
    if ( this._groups.hasGroup(name) ) throw new Error(`Group already has a group named ${name}`)
    return this.tasks[name] = new GulpGroupTask(name, this)
  }

  // and possibly groups
  group(name, ...args){
    if ( this.tasks[name] ) throw new Error(`Group already has a task named ${name}`)
    return this._groups.group(name, ...args)
  }
