...
class DeployableAssets extends Assets {
  ...
  static setup( options = {} ){
    ...
    if (options.cwd) dgulp.setCwd(options.cwd)
    if (options.debug) dgulp.setDebug(options.debug)

    // Set the default destination for others to append to
    let src_path = options.src || 'app/assets'
    dgulp.setSourcePath(src_path)

    let dist_path = options.dist || 'app/static/assets'
    dgulp.setDistPath(dist_path)

    let public_url = options.public || '/assets'
    dgulp.setPublicUrl(public_url)

    debug('src:%s dist:%s public:%s', src_path, dist_path, public_url)

    // `assets` sourcing

    dgulp.createAssetsJqueryNode(dist_path)
    dgulp.createAssetsBootstrap3SassNode(src_path, dist_path)
