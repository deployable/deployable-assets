...
class DeployableAssets extends Assets {
  ...
  setupBootstrap3( group, options = {} ){
    ...
    if (options.dest) tsk_font.setDest(path.join(options.dest, 'fonts','bootstrap'))
    else tsk_font.setDestSuffix(path.join(this.dir_font,'bootstrap'))
  ...
  setupBootstrap3Sass( group, src_prefix ){
    ...
    group.task('fonts')
      ...
      .setDestSuffix(path.join(this.dir_font,'bootstrap'))
      .createCopyTask()
