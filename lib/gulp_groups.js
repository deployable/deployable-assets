const debug = require('debug')('dply:assets:gulp_groups')

const { GulpGroup } = require('./gulp_group')
const { GulpDestMixin } = require('./gulp_dest_mixin')
const { isUndefined, flatMap, map, forIn } = require('lodash')

class GulpGroups  extends GulpDestMixin {

  static init(){}

  constructor( name, parent, rootgulp, options = {} ){
    super()
    this.name = name
    this.groups = {}
    this.parent = parent
    this.root = rootgulp
    if ( ! isUndefined(options.debug) ) this.debug = options.debug
  }
